...
Mon directeur de recherche Emmanuel Château-Dutier, m'a donné la possibilité de reprendre sa charge de cours "Introduction aux métadonnées", pour les étudiants de la maîtrise en muséologie. Je vous propose donc ici, en m'inspirant de son cours, un petit article définissant les données, métadonnées et leurs modélisations. 
...
ouverture sur l'utilisation des données - ia ?
...
Au fil du temps, quatre grands modèles de données se sont imposés, chacun avec ses forces et ses limites. <!--(#REF Baca)-->[^11]
