...
## 2. What solution do we have when exhibition destabilize authorship, identity, and temporal boundaries?
...
The Curate ontology offers one useful distinction here: story, a set of factual events, and plot, a subjective interpretation that imposes relations like causality onto those events. Because the same events can be interpreted differently, Curate allows multiple plots over one story, each with its own justification. That distinction becomes essential the moment we look at how the different actors around *Feux pâles* actually interpret the same set of events, which is exactly what the next slide lays out.
...
### Curate applied to *Feux pâles*: story, plot, narrative
...
### AAAo: classificatory status, applied to the barcode's authorship
...
XX:XX_Undisclosed a owl:Class ;
    rdfs:subClassOf crm:E13_Attribute_Assignment ;
    rdfs:subClassOf crm:E73_Information_Object ;  # documented reason for withholding
