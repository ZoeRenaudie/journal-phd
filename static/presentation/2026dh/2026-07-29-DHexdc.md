...
The “collections as data” paradigm proposes that we stop treating digital surrogates as mere stand-ins for physical documents, and start treating them as raw material: something computers can process, query, and recombine.<!-- @ec dans le texte, je ferais tout de même une mention de Padilla, en ramenant en une phrase le contexte dans lequel il développe cette notion. Je comprends que tu ne dises pas immédiatement pq tu y as recours mais c’est un peu abrupt. -->
...
Unlike the standard document collections of a library<!-- @ec ou des musées, même si c’est contestable. Ou ajouter après la phrase une rmq sur le fait que c’est pire que dans les œuvres muséales. -->, exhibitions are what I would call small, complex, and difficult data. They are not made of stable entities, but of dynamic relationships: between artworks, space, infrastructure, and embodied experience. <!-- @ec Attention, tu glisses de document à données dans le § -->

Our current information systems <!-- @ec in museums --> are built for the opposite: they want finished objects, single authors, fixed dates. This creates friction, especially for exhibitions rooted in feminist, queer, or community practices, which resist exactly these norms. As Johanna Drucker reminds us, data is never raw: it is *capta*, something taken, constructed, interpreted.
...
<!-- @ec Populer, n’est-ce pas construire un dataset ? Je réunirais. Tu pourrais parler du caractère itératif du processus. Peut-être ajouter la mise au point d’une proposition ontologique -->
...
```
<figcaption> Representation of the Exhibition Concept (c) Zoë Renaudie. Tool : mermaid <!-- @ec
Pourquoi un © ? Des crédits suffisent, d’autant que le régime du copyright n’est ni du droit français, ni réellement du droit canadien. Le titre de la représentation me paraît inadéquate --></figcaption>
</figure>
...
<div class="two-col" style="display: flex; gap: 1rem;">
  <div class="flex-1">
    <figure>
...
*ICOM, Version 7.3.2, 2026*
</div> <div class="card">

**LRMoo (FBROO)** Work, Expression, Manifestation, Item.
*ICOM and IFLA, Version 1.1.1, 2025*
</div> <div class="card">

OntoExhibit : Exhibition discursive dimensions.
*iArtHist Lab, Valencia University, Version 1.1.0, 2026*
</div> <div class="card">

**Curate** Separates story from plot.
*Knowledge Media Institute, The Open University, Version 2012*
</div> <div class="card">

Display : extension for topological description of exhibition display using BOT.
*Ouvroir Lab, University of Montreal, Version 0.1.0 2024*
</div> <div class="card">

**AAAo** Institutional-fact model for difficult historical data.
*Sari and TakinSolutions, Version 1.7.1 2025*
</div> </div> <br/>
...
Depuis les années 90, les institutions culturelles ont développé plusieurs modèles documentaires pour rendre compte de l’information muséale. Le modèle conceptuel de référence développé par le groupe de documentation de l’International Council of Museums (ICOM), le CIDOC-CRM, propose par exemple une ontologie générique, orientée événement, qui permet de décrire la vie des objets culturels. Nicola Carboni a beaucoup travailler sur CIDOC pour ... completer....  Curate et le Onto-Exhibit s’intéresse quant à la dimension discursive de l’exposition. Nous avons développé Display un modèle spécialisé pour la documentation spatiale des accrochages d’exposition ou de collection. L’extension AAAo (Art and Architectural Argumentation Ontology) tente de modéliser les « données historiques difficiles » sans les réduire. 

The ones with bold font are the ones I will return to in the following slides. 

Parce que bien que Nicolas et Nuria aient permis la représentation d'un grande partie des informations sur une exposition, ma recherche documentaire de conservation a rencontrer des difficultés en voulant documenter certains point. 
...
First question : 
Treated as a chain of activations rather than a static entity, *Feux pâles* extends from the original 1990 exhibition to the *L’Ombre du jaseur* reactivation in 2014, passing through the *Cabinet d’amateur* and the conservation study itself. Following Nelson Goodman’s notion of worldmaking, exhibition documentation must support the coexistence of multiple epistemic perspectives without reducing them to a single unified interpretation.
...
Thanks to Carboni and SARI I can map a lot of information on the exhibition. But LRMoo adds a proper four-level hierarchy on top, from Work down to Item to express the activations we mentionned. <!-- @ec la réparatition entre ce qui serait une expression, manifestation, item est discutable. Pour moi le catalogue serait soit une expression soit une manifestation. Dans les définitions de FRBR les items étaient plutôt destinés aux exemplaires bibliographiques qui pouvaient porter des marques de propriétaires, des annotations, etc. Je n’ai pas en tête la définition donnée par LRM. Mais ce dispositif était initialement pour les œuvres multiples et tu pourrais en avoir beosin pour les catalogues justement. -->
...
**Emeline Jaret** Reconstitution requires rigorous documentary accompaniment. 
...
<!-- @ec je suppose que d’autres choses pourraient également être utiles dans AAAo pour modéliser les aspects performatifs de l’exposition qu’a cherché à traiter Nuria. Dans l’exemple que tu proposes, je trouve intéressant le fait que l’œuvre conceptuelle qui dans LRM devrait pouvoir donner lieu à des expressions manifestations, etc. soit décrite comme non reproductible. Il y a peut-être qqch à creuser là plus tard sur ce que signifie l’utilisation de certaines classes et le rapport au discours qui est tenu sur elles. -->
...
*I see an object (`crm:Human-made_Object`, a cup) in an exhibition photograph, and I believe it corresponds to an entry in the catalogue entry (“antique cup”), but only plausibly, not with certainty.*
...
<!-- @ec sans doute pour plus tard, mais est-ce que AAAo ne règle pas la question en travaillant sur les *claims* plutôt que la logique inférentielle ?Quel est le point de vue de AAAo sur CRMInf ? comment la situent-ils ? est-ce qu’ils l’utilisent ? -->
...
<!-- @ec ici ou dans la conclusion ramener les travaux plus généraux en cours sur les expositions et replacer ta réflexion -->
